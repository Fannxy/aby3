...
taskset -c 0-$((${task_num} - 1)) mpirun -np $task_num $USER_FOLDER/out/build/linux/frontend/frontend -role 0 ${args_list} >> ./log 2>&1 &
ssh aby31 "ulimit -n 65536; taskset -c 0-$((${task_num} - 1)) mpirun -np $task_num $USER_FOLDER/out/build/linux/frontend/frontend -role 1 ${args_list} &" &
ssh aby32 "ulimit -n 65536; taskset -c 0-$((${task_num} - 1)) mpirun -np $task_num $USER_FOLDER/out/build/linux/frontend/frontend -role 2 ${args_list} &" &

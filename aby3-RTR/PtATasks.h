...
template <typename NUMX, typename NUMY, typename NUMT, typename NUMR>
class PtAMBSAB : public PairOnlySubTask<NUMX, NUMY, NUMT, NUMR> {
public:
    //aby3 info
    int pIdx;
    aby3::Sh3Encryptor* enc;
    ...
    PtAMBSAB(const size_t optimal_block, const int task_id, const int pIdx,
            aby3::Sh3Encryptor& enc, aby3::Sh3Runtime& runtime,
            aby3::Sh3Evaluator& eval) : 
        pIdx(pIdx),
        enc(&enc),
    ...
    virtual void compute_local_table(std::vector<NUMX>& expandX, std::vector<NUMY>& expandY, std::vector<NUMT>& local_table, BlockInfo* binfo) override {
        ...
        // trans to the arithmetric table.
        aby3::si64Matrix arith_diff_table(binfo->block_len, 1);
        aby3::si64Matrix ones(binfo->block_len, 1);
        init_ones(this->pIdx, *(this->enc), *(this->runtime), ones, binfo->block_len);
        arith_bool_mul(this->pIdx, ones, diff_table, arith_diff_table, *(this->enc), *(this->eval), *(this->runtime));

        for(size_t i=0; i<binfo->block_len; i++){
            local_table[i].first.mData[0] = arith_diff_table.mShares[0](i, 0);
            local_table[i].first.mData[1] = arith_diff_table.mShares[1](i, 0);
            local_table[i].second.mData[0] = diff_table.mShares[0](i, 0);
            local_table[i].second.mData[1] = diff_table.mShares[1](i, 0);
